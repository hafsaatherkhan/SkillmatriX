...
export default function Navbar({ showAuthButtons = true, isLoggedIn = false }) {
    ...
    <nav className={baseClasses}>
      ...
      <div className={innerClasses}>
        ...
        <div className="relative flex items-center group cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
          ...
          <span className="text-2xl font-black text-[#3D418A] tracking-tighter">
            SkillmatriX
          </span>
        </div>
        ...
        <div className="flex items-center gap-4">
          {showAuthButtons && !isLoggedIn && (
            <>
              <Button
                variant="ghost"
                className="!px-6 !py-3 !text-sm hidden sm:flex min-w-[100px]"
              >
                Log In
              </Button>
              <Button variant="primary" className="!px-6 !py-3 !text-sm min-w-[100px]">
                Sign Up
              </Button>
            </>
          )}
          {isLoggedIn && <Button variant="primary" className="!px-6 !py-3 !text-sm">Log Out</Button>}
        </div>
      </div>
    </nav>
  );
